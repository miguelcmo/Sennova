...
if ($check !== false) {
    ...
    if (move_uploaded_file($_FILES["upload"]["tmp_name"], $targetFile)) {
        ...
        echo json_encode(["url" => "http://localhost/sennova/serviserBackend/web/ckeditor/" . $targetFile]);
    } else {
...
// Configuración de la carpeta donde se guardarán las imágenes
$targetDir = Yii::getAlias('@webroot/images/');  // Ruta física donde se guardarán las imágenes
if (!is_dir($targetDir)) {
    mkdir($targetDir, 0777, true);  // Crea la carpeta si no existe
}
...
if ($check !== false) {
    ...
    if (move_uploaded_file($_FILES["upload"]["tmp_name"], $targetFile)) {
        ...
        $imageUrl = Url::to('@web/images/' . basename($targetFile), true);
